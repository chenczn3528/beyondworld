...
def wiki_detailed_info(card_name):
    ...
    # 获取文本内容
    text = [t for t in page.text().split("{{侧影") if t.strip()]
...
for index, row in enumerate(rows):
    ...
    info_dict = wiki_detailed_info(name_div.get_text(strip=True).split("·")[-1])
    ...
    if link_tag:
        ...
        try:
            print(f"[{index}] 正在抓取：{info_dict['名称']} 的详情页内容")
